...
class CenterHead(BaseModule):
    ...
    def get_targets_mask(self, gt_bboxes_3d, gt_labels_3d):
        ...
        for i in range(len(heatmaps)):
            #print("i: ",i, heatmaps[i].shape)
            if heatmaps[i].shape[1]>1:
                for j in range(heatmaps[i].shape[1]):
                    #print("j: ",j, heatmaps[i][:,j,:,:].shape)
                    overallHeatmap += heatmaps[i][:,j,:,:].unsqueeze(1)
            #print("i: ",i, heatmaps[i].shape)
            else:
                overallHeatmap += heatmaps[i]
            #print("overallHeatmap shape", overallHeatmap.shape)
        # Transpose anno_boxes
        # anno_boxes = list(map(list, zip(*anno_boxes)))
        # anno_boxes = [torch.stack(anno_boxes_) for anno_boxes_ in anno_boxes]
        # # Transpose inds
        ...
        Returns:
            tuple[list[torch.Tensor]]: Tuple of target including \
                ...
                - list[torch.Tensor]: Masks indicating which boxes \
                    are valid.
        """
        #print("in centerhead loss: ", type(gt_labels_3d))
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, preds_dicts_teacher, **kwargs):
        ...
        heatmapformask = self.get_targets_mask(gt_bboxes_3d,gt_labels_3d)
        loss_dict = dict()
        for task_id, preds_dict in enumerate(preds_dicts):
            # heatmap focal loss
            if len(preds_dicts_teacher) > 0:
                preds_dict_teacher = preds_dicts_teacher[task_id]
                preds_dict_teacher[0]["heatmap"] = clip_sigmoid(preds_dict_teacher[0]["heatmap"])
            ...
            if len(preds_dicts_teacher) > 0:
                preds_dict_teacher[0]["anno_box"] = torch.cat(
                    (
                        preds_dict_teacher[0]["reg"],
                        preds_dict_teacher[0]["height"],
                        preds_dict_teacher[0]["dim"],
                        preds_dict_teacher[0]["rot"],
                        preds_dict_teacher[0]["vel"],
                    ),
                    dim=1,
                )
            ...
            if len(preds_dicts_teacher) > 0:
                pred_teacher = preds_dict_teacher[0]["anno_box"].permute(0, 2, 3, 1).contiguous()
                pred_teacher = pred_teacher.view(pred_teacher.size(0), -1, pred_teacher.size(3))
                pred_teacher = self._gather_feat(pred_teacher, ind)
            ...
            if len(preds_dicts_teacher) > 0:
                loss_dict[f"heatmap/task{task_id}_kd_cls_loss"] = self.get_cls_layer_loss \
                    (preds_dict_teacher[0]["heatmap"], preds_dict[0]["heatmap"], self.num_classes[task_id])
                # loss_dict[f"bbox/task{task_id}_kd_bbox_loss"] = self.get_box_reg_layer_loss \
                #     (pred_teacher, pred, preds_dict_teacher[0]["heatmap"], self.num_classes[task_id])
                loss_dict[f"bbox/task{task_id}_kd_bbox_loss"] = self.get_box_reg_layer_loss \
                    (pred_teacher, pred, bbox_weights, self.num_classes[task_id])
        return loss_dict, heatmapformask
    ...
    def get_cls_layer_loss(self, teacher_cls_preds, student_cls_preds, num_class):

        cls_kd_losses = 0.0
        for i in range(len(teacher_cls_preds)):
            cls_preds = student_cls_preds[i]
            batch_size = cls_preds.shape[0]
            cls_preds = cls_preds.view(batch_size, -1, num_class).contiguous()
            # teacher pred
            cls_teacher_pred = teacher_cls_preds[i]
            cls_teacher_pred = cls_teacher_pred.view(batch_size, -1, num_class).contiguous()
            # to logits
    ...
    # def get_box_reg_layer_loss(self, teacher_box_preds, student_box_preds, teacher_cls_preds, num_class):
    #     loc_losses = 0.0
    ...
    #         # B,H,W,N_cls -> B,H*W
    #         positives = cls_teacher_pred.view(batch_size, -1, num_class).contiguous().sum(-1) > 0.1
    #         reg_weights = positives.float()
    #         pos_normalizer = positives.sum(1, keepdim=True).float()
    #         reg_weights /= torch.clamp(pos_normalizer, min=1.0)
    #         reg_weights *= cls_teacher_pred.view(batch_size, -1, num_class).contiguous().sum(-1)
    #
    ...
    #     box_loss = loc_loss
    #     return box_loss
    def get_box_reg_layer_loss(self, teacher_box_preds, student_box_preds, bbox_weights, num_class):
        loc_losses = 0.0
        for i in range(len(teacher_box_preds)):
            # student pred
            box_preds = student_box_preds[i]
            ...
            # sin(a - b) = sinacosb-cosasinb
            box_preds_sin, reg_targets_sin = self.add_sin_difference(box_preds, box_teacher_pred)
            #print("box_preds_sin", box_preds_sin.shape)
            #print("bbox_weights", bbox_weights.shape)
            loc_loss_src = self.reg_loss_func(box_preds_sin, reg_targets_sin)#, weights=bbox_weights)  # [N, M]
