...
class TransFusionHead(nn.Module):
    ...
    @force_fp32(apply_to=("preds_dicts", "preds_dicts_student"))
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, preds_dicts_student, **kwargs):
        ...
        (
            labels_student,
            label_weights_student,
            bbox_targets_student,
            bbox_weights_student,
            ious_student,
            num_pos_student,
            matched_ious_student,
            heatmap_student,
        ) = self.get_targets(gt_bboxes_3d, gt_labels_3d, preds_dicts_student[0])
        if hasattr(self, "on_the_image_mask"):
            label_weights = label_weights * self.on_the_image_mask
            bbox_weights = bbox_weights * self.on_the_image_mask[:, :, None]
            num_pos = bbox_weights.max(-1).values.sum()
        if hasattr(self, "on_the_image_mask"):
            label_weights_student = label_weights_student * self.on_the_image_mask
            bbox_weights_student = bbox_weights_student * self.on_the_image_mask[:, :, None]
            num_pos_student = bbox_weights_student.max(-1).values.sum()
        preds_dict = preds_dicts[0][0]
        preds_dict_student = preds_dicts_student[0][0]
        loss_dict = dict()
        # cls_preds = []
        # box_preds = []
        ...
        loss_heatmap_student = self.loss_heatmap(
            clip_sigmoid(preds_dict_student["dense_heatmap"]),
            heatmap_student,
            avg_factor=max(heatmap_student.eq(1).float().sum().item(), 1),
        )
        loss_dict["loss_heatmap_student"] = loss_heatmap_student
        ...
        for idx_layer in range(self.num_decoder_layers if self.auxiliary else 1):
            ...
            layer_labels_student = labels_student[
                           ...,
                           idx_layer * self.num_proposals: (idx_layer + 1) * self.num_proposals,
                           ].reshape(-1)
            layer_label_weights_student = label_weights_student[
                                  ...,
                                  idx_layer * self.num_proposals: (idx_layer + 1) * self.num_proposals,
                                  ].reshape(-1)
            layer_score_student = preds_dict_student["heatmap"][
                          ...,
                          idx_layer * self.num_proposals: (idx_layer + 1) * self.num_proposals,
                          ]
            layer_cls_score_student = layer_score_student.permute(0, 2, 1).reshape(-1, self.num_classes)
            layer_loss_cls_student = self.loss_cls(
                layer_cls_score_student,
                layer_labels_student,
                layer_label_weights_student,
                avg_factor=max(num_pos_student, 1),
            )
            ...
            layer_center_student = preds_dict_student["center"][
                           ...,
                           idx_layer * self.num_proposals: (idx_layer + 1) * self.num_proposals,
                           ]
            layer_height_student = preds_dict_student["height"][
                           ...,
                           idx_layer * self.num_proposals: (idx_layer + 1) * self.num_proposals,
                           ]
            layer_rot_student = preds_dict_student["rot"][
                        ...,
                        idx_layer * self.num_proposals: (idx_layer + 1) * self.num_proposals,
                        ]
            layer_dim_student = preds_dict_student["dim"][
                        ...,
                        idx_layer * self.num_proposals: (idx_layer + 1) * self.num_proposals,
                        ]
            preds_student = torch.cat(
                [layer_center_student, layer_height_student, layer_dim_student, layer_rot_student], dim=1
            ).permute(
                0, 2, 1
            )  # [BS, num_proposals, code_size]
            if "vel" in preds_dict_student.keys():
                layer_vel_student = preds_dict_student["vel"][
                            ...,
                            idx_layer
                            * self.num_proposals: (idx_layer + 1)
                                                  * self.num_proposals,
                ...
                preds_student = torch.cat(
                    [layer_center_student, layer_height_student, layer_dim_student, layer_rot_student, layer_vel_student], dim=1
                ).permute(
                    0, 2, 1
                )  # [BS, num_proposals, code_size]
            code_weights = self.train_cfg.get("code_weights", None)
            layer_bbox_weights_student = bbox_weights_student[
                                 :,
                                 idx_layer * self.num_proposals: (idx_layer + 1) * self.num_proposals,
                                 :,
                                 ]
            layer_reg_weights_student = layer_bbox_weights_student * layer_bbox_weights_student.new_tensor(
                code_weights
            )
            layer_bbox_targets_student = bbox_targets_student[
                                 :,
                                 idx_layer * self.num_proposals: (idx_layer + 1) * self.num_proposals,
                                 :,
                                 ]
            layer_loss_bbox_student = self.loss_bbox(
                preds_student, layer_bbox_targets_student, layer_reg_weights_student, avg_factor=max(num_pos_student, 1)
            )
            ...
            #loss_dict[f"{prefix}_loss_cls_student"] = layer_loss_cls_student
            loss_dict[f"{prefix}_loss_bbox"] = layer_loss_bbox
            #loss_dict[f"{prefix}_loss_bbox_student"] = layer_loss_bbox_student
            #loss_dict[f"{prefix}_kd_cls_loss"] = self.get_cls_layer_loss(layer_score, layer_score_student)
            #loss_dict[f"{prefix}_kd_bbox_loss"] = self.get_box_reg_layer_loss(preds,preds_student,layer_score)

            # loss_dict[f'{prefix}_loss_iou'] = layer_loss_iou
            # cls_preds.append(layer_score)
            # box_preds.append(preds)

        loss_dict[f"matched_ious"] = layer_loss_cls.new_tensor(matched_ious)
        #loss_dict[f"matched_ious_student"] = layer_loss_cls_student.new_tensor(matched_ious_student)
    ...
    def get_cls_layer_loss(self, teacher_cls_preds, student_cls_preds):

            cls_kd_losses = 0.0
            for i in range(len(teacher_cls_preds)):
                cls_preds = student_cls_preds[i]
                batch_size = cls_preds.shape[0]
                cls_preds = cls_preds.view(batch_size, -1, self.num_classes).contiguous()
                # teacher pred
                cls_teacher_pred = teacher_cls_preds[i]
                cls_teacher_pred = cls_teacher_pred.view(batch_size, -1, self.num_classes).contiguous()
                # to logits
                ...
                cls_kd_losses += cls_loss
            # cls_loss_return = cls_kd_losses/len(teacher_cls_preds)
            cls_weight = 0.25
            cls_loss_return = cls_kd_losses * cls_weight

            return cls_loss_return

    def get_box_reg_layer_loss(self, teacher_box_preds, student_box_preds, teacher_cls_preds):
            loc_losses = 0.0
            for i in range(len(teacher_box_preds)):
                # student pred
                box_preds = student_box_preds[i]
                # box_dir_cls_preds = self.forward_ret_dict.get('dir_cls_preds', None)
                batch_size = box_preds.shape[0]

                # teacher pred
                box_teacher_pred = teacher_box_preds[i]
                # box_dir_cls_teacher_preds = self.teacher_pred.get('dir_cls_preds', None)

                # pos norm
                cls_teacher_pred = torch.sigmoid(teacher_cls_preds[i])
                # B,H,W,N_cls -> B,H*W
                positives = cls_teacher_pred.view(batch_size, -1, self.num_classes).contiguous().sum(-1) > 0.1
                reg_weights = positives.float()
                pos_normalizer = positives.sum(1, keepdim=True).float()
                reg_weights /= torch.clamp(pos_normalizer, min=1.0)
                reg_weights *= cls_teacher_pred.view(batch_size, -1, self.num_classes).contiguous().sum(-1)
                ...
                loc_losses += loc_loss
            cls_weight = 0.25
            loc_loss = loc_losses * cls_weight
            box_loss = loc_loss
            return box_loss
