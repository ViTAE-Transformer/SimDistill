...
def unified_focal_loss(prob_volume, depth_values, interval, depth_gt, mask, weight, gamma, alpha):
    ...
    pos_weight = (torch.sigmoid((gt_unity - prob_volume).abs() / gt_unity, base=5) - 0.5) * 4 + 1  # [1, 3]
    neg_weight = (torch.sigmoid(prob_volume / gt_unity, base=5) - 0.5) * 2  # [0, 1]
...
class BEVFusion_bevformer(Base3DFusionModel):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        print(type(encoders["camera"]["student_pts_bbox_head"]))
        self.encoders_student = nn.ModuleDict()
        if encoders.get("camera") is not None:
            self.encoders_student["camera"] = nn.ModuleDict(
                {
                    "backbone": build_backbone(encoders["camera"]["student_backbone"]),
                    "neck": build_neck(encoders["camera"]["student_neck"]),
                    "student_pts_bbox_head": build_head(encoders["camera"]["student_pts_bbox_head"]),
                }
            )
    ...
    def train(self, mode=True):
        ...
        super(BEVFusion_bevformer, self).train(mode)
        if self.freeze:
    ...
    def extract_camera_features_student(
        ...
        out = self.encoders_student["camera"]["student_pts_bbox_head"](
            img_feats_reshaped, img_metas)
        bevfeat = out["bev_embed"]
        return bevfeat, Lift2
        ...
        **kwargs,
        ...
        for sensor in (
          ...
          #      print("camera feature size: ", feature.shape)
                feature_student_Lift1, feature_student_Lift2 = self.extract_camera_features_student(
                    img,
                    points,
                    depth_gt,
                    camera2ego,
