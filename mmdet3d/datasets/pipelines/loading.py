...
class LoadMultiViewImageFromFiles:
    ...
    def __call__(self, results):
        ...
        #    print(simg.size)
            if simg.size != (1600, 900) and i < len(filename):
                simg = Image.open(filename[i+1])
                print(simg.size)
                print(name)
            elif simg.size != (1600, 900) and i > 0:
                simg = Image.open(filename[i - 1])
                print(simg.size)
                print(name)
        ...
        images= img_list
        for i in range(len(filename)):
            rootdir = "/docker-home/data/Nuscenes/"
            imgname = filename[i].split("/")[-1]
            name = rootdir + "depth_gt/" + imgname + ".bin"
            dimg = np.fromfile(name, dtype=np.float32).reshape(-1, 3)
            #            print("loading depth size$$$$$$$$$$$$$$$$$$$$$$$$$$$$",dimg.shape)
            #            print("$$$$$$$$$$$$$$$",dimg[0].shape, dimg[1].shape,dimg[2].shape)
            #            print("depth max valuse0***********",np.max(dimg[:,0]))
            #            print("depth min valuse0***********",np.min(dimg[:,0]))
            #            print("depth max valuse1***********",np.max(dimg[:,1]))
            #            print("depth min valuse1***********",np.min(dimg[:,1]))
            #            print("depth max valuse2***********",np.max(dimg[:,2]))
            #            print("depth min valuse2***********",np.min(dimg[:,2]))
            #            depth_map = np.zeros((900,1600))
            #            dimgcoords = dimg[:,:2].astype(np.uint8)
            #            #dimg[:,:2] = dimgcoords
            #            depth_map[dimgcoords[:,0],dimgcoords[:,1]] = dimg[:,2]
            #            print("new depth size", depth_map.shape)
            #            print("depth max valuse***********",np.max(depth_map))
            #            print("depth min valuse***********",np.min(depth_map))
            #            #print("----depthmap max valuse0***********",np.max(depth_map[:,0]))
            #            #print("----depthmap min valuse0***********",np.min(depth_map[:,0]))
            #            #print("----depthmap max valuse1***********",np.max(depth_map[:,1]))
            #            #print("----depthmap min valuse1***********",np.min(depth_map[:,1]))
            #            #print("----depthmap max valuse2***********",np.max(depth_map[:,2]))
            #            #print("----depthmap min valuse2***********",np.min(depth_map[:,2]))
            #            #depthmap = Image.fromarray(np.uint8(cm.gist_earth(depth_map)*255))
            #            depthmap = Image.fromarray(np.uint8(depth_map))
            #            depth_list.append(depthmap)
            depth_list.append(dimg)
        depths = depth_list
...
@PIPELINES.register_module()
class LoadLidarDepth:
    """Load Depth generated from Lidar.
    ...
    def __call__(self, results):
        ...
        # img is of shape (h, w, c, num_views)
        # modified for waymo
        # depth_filename = results["image_paths"] + ".bin"
        images = []
        img_list = []
        h, w = 0, 0
        # for name in filename:
        ...
        for i in range(len(filename)):
            ...
            img_list.append(simg)
        images = img_list
        # images= np.stack(img_list, axis=-1)
        # TODO: consider image padding in waymo

        # results["filename"] = filename
        # unravel to list, see `DefaultFormatBundle` in formating.py
        # which will transpose each image separately and then stack into array
        results["depth_gt"] = images
        #print("loading from depth----------------")
        return results
