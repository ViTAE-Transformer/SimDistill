...
class ImageAug3D:
    ...
    def depth_transform(
            self, cam_depth, rotation, translation, ori_shape, resize, resize_dims, crop, flip, rotate
    ):
        """Transform depth based on ida augmentation configuration.

            Args:
                cam_depth (np array): Nx3, 3: x,y,d.-->HW1
        ...
        #        print("cam_depth type", type(cam_depth))
        #        print("cam_depth shape", cam_depth.size)
        #        print("cam_depth value max", np.max(cam_depth))
        #        camd_rot = cam_depth.rotate(rotate)
        #        print("cam_rot shape", camd_rot.size)
        #        print("cam_rot value max", np.max(camd_rot))
        #        print("---------------------------------------")
        #        print("cam_rot", camd_rot)
        #        #camd_resize = F.interpolate(camd_rot,resize_dims)
        #        #camd_resize = camd_rot.resize(resize_dims,Image.ANTIALIAS)
        #        from torchvision import transforms
        #        resize_full = transforms.Resize(resize_dims, interpolation=Image.ANTIALIAS)
        #        camd_resize = resize_full(camd_rot)
        #        print("---------------------------------------")
        #        print("cam_resize", camd_resize)
        #        print("---------------------------------------")
        #        print("camd_resize value max", np.max(camd_resize))
        #        print("camd_resize shape", camd_resize.size)
        #        img = camd_resize.crop(crop)
        #        print("camd_crop value max", np.max(img))
        #        print("camd_crop shape", img.size)
        #
        #        if flip:
        #            img = img.transpose(method=Image.FLIP_LEFT_RIGHT)
        #        # post-homography transformation
        ...
        #            rotation = A.matmul(rotation)
        #            translation = A.matmul(translation) + b
        #        print("depth transform shape", img.size)
        #        #arrayd =
        #        print("transform3d max value", np.max(img))
        #        return img

        # cam_depth = np.array(cam_depth)
        # print("cam_depth type", type(cam_depth))
        # print("cam_depth shape", cam_depth.shape)
        # print("&&&&&&&&&&&&&&&cam_depth",np.max(cam_depth[:,2]))
        H, W = resize_dims
        cam_depth[:, :2] = cam_depth[:, :2] * resize
        # print("cam_depth resize shape", cam_depth.shape)
        # print("&&&&&&&&&&&&&&&cam_depth resize",np.max(cam_depth[:,2]))
        cam_depth[:, 0] -= crop[0]
        cam_depth[:, 1] -= crop[1]
        # print("cam_depth crop shape", cam_depth.shape)
        # print("&&&&&&&&&&&&&&&cam_crop",np.max(cam_depth[:,2]))
        if flip:
        ...
        # print("depth_coords shape", depth_coords.shape)
        ...
        # print("&&&&&&&&&&&&&&&depth_map)",np.max(depth_map))
        # depth_tensor = torch.Tensor(depth_map)
        depthmap = depth_map
        # depthmap = Image.fromarray(np.uint8(depth_map))
        # print("depth_tensor shape", depthmap.size)
        # print("&&&&&&&&&&&&&&&depthmap",np.max(depthmap))
        return depthmap

    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        imgs = data["img"]
        depths = data["depth_gt"]
        ...
        for i in range(len(imgs)):
            ...
            new_depth = self.depth_transform(
                depth,
                post_rot,
                post_tran,
                ori_shape,
                resize=resize,
                resize_dims=self.final_dim,
                crop=crop,
                flip=flip,
                rotate=rotate,
            )
...
class ImageNormalize:
    def __init__(self, mean, std):
        ...
        self.compose_depth = torchvision.transforms.Compose(
            [
                torchvision.transforms.ToTensor(),
            ]
        )
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        data["depth_gt"] = [self.compose_depth(depth) for depth in data["depth_gt"]]
        return data
